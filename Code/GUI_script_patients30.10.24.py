# -*- coding: utf-8 -*-
"""
Created on Sun Jun  2 09:13:11 2024

@author: ferbe
"""
# -*- coding: utf-8 -*-
"""
Created on Tue May 28 20:29:28 2024

@author: ferbe
"""
# -*- coding: utf-8 -*-
"""
Created on Mon May 27 19:14:37 2024
...
from data_patients import data_patients
...
class main:
    def __init__(self,num_patients=15):
        self.initial_patients = data_patients().patients
        
        self.set_order4image()
    ...
    def set_imgAnalysis(self,dic):
        analysed_patient_images = self.initial_patients
...
class ReportCoronaries(QMainWindow):
    def __init__(self,identifier,main_class):
        ...
        self.CADRADS_options = ['0','1','2','3','4A','4B','5','N']
    ...
    def set_combobox_selection(self):
        if self.ident in self.main_class.get_imgAnalysis().keys():
            ...
            try:
                for i in range(len(self.database_segments)):
                    for j in self.main_class.get_imgAnalysis()[self.ident]['Plaque segments']:
                        if self.main_class.get_imgAnalysis()[self.ident]['Plaque segments'][j][1] == self.database_segments[i]:
                            self.report_segments_plaque[i].setCurrentIndex(self.plaque_type.index(self.main_class.get_imgAnalysis()[self.ident]['Plaque segments'][j][0]))
                            break
                        elif j == sorted(self.main_class.get_imgAnalysis()[self.ident]['Plaque segments'].keys())[-1]:
                            self.report_segments_plaque[i].setCurrentIndex(0)
                            self.report_segments_plaque[i].setStyleSheet("background-color: grey; color: white;")
                    self.changed_color_plaque(self.report_segments_plaque[i])
                    self.changed_plaque(self.report_segments_plaque[i])
            except:
                pass
    ...
    def update_records(self):
        ...
        if self.ident in self.main_class.get_imgAnalysis().keys():
            ...
            try:
                plaque_reported = []
                for j in self.main_class.get_imgAnalysis()[self.ident]['Plaque segments']:
                    plaque_reported.append(self.main_class.get_imgAnalysis()[self.ident]['Plaque segments'][j][1])
                    index_name = self.name_vessels.index(self.main_class.get_imgAnalysis()[self.ident]['Plaque segments'][j][1])
                    self.main_class.get_imgAnalysis()[self.ident]['Plaque segments'][j][0] = self.plaque_type[self.report_segments_plaque[index_name].currentIndex()]
                
                plaque_unreported = [x for x in self.name_vessels if x not in plaque_reported]
                            
                counter = 1
                for i in range(len(plaque_unreported)):
                    index_name = self.name_vessels.index(plaque_unreported[i])
                    if self.report_segments_plaque[index_name].currentIndex() != 0:
                        self.main_class.get_imgAnalysis()[self.ident]['Plaque segments'][int(j)+counter]=[self.plaque_type[self.report_segments_plaque[index_name].currentIndex()],plaque_unreported[i]]
                        counter += 1
            except:
            ...
            #print('UPDATED PP: ',self.main_class.get_imgAnalysis()[self.ident])
        ...
        #UPDATE NON-AI REPORTS
            ...
            #print('REPORTED PP: ',self.main_class.get_filtering()[1][self.ident])
