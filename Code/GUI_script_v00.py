...
from PyQt5.QtWidgets import QMainWindow, QApplication, QMessageBox, QPushButton, QLabel, QLineEdit, QFileDialog, QVBoxLayout, QWidget, QSlider, QDockWidget
...
class Search(QMainWindow):
    ...
    def search_patient(self):
        ...
        # admin['Center'] = self.textHospital.text()
        # admin['Service'] = self.textService.text()
        # admin['Study date'] = str(self.dateStudy.date())
        
        # ident = str(self.textID.text())
        
        # self.main_class.initial_patients[ident] = {}
        # self.main_class.initial_patients[ident]['EHR'] = EHR
        # self.main_class.initial_patients[ident]['admin'] = admin
        # QMessageBox.information(self, "Successful registration", 'The patient has been successfully registered') 
        # print('NEW PATIENT: ',self.main_class.initial_patients[ident])    
...
#class LogIn(QWidget):
    def __init__(self, main_class):
        super().__init__()
        uic.loadUi('gui_worklist.ui',self)
        ...
        for i in range(num_patients):
            ...
            num_order.setAlignment(QtCore.Qt.AlignCenter)
            ident = QLabel(order[i][0])
            urgency = QLabel(order[i][1])
            cadrads = QLabel(order[i][2])
            cadrads.setAlignment(QtCore.Qt.AlignCenter)
            prepro = QLabel(str(order[i][3][0]))
            prepro.setAlignment(QtCore.Qt.AlignCenter)
            cacs = QLabel(str(order[i][4]))
            cacs.setAlignment(QtCore.Qt.AlignCenter)
            pretest = QLabel(str(order[i][5]))
            pretest.setAlignment(QtCore.Qt.AlignCenter)
            motive = QLabel(order[i][6])
            urg_std = QLabel(str(order[i][7]))
            ...
            if order[i][1] == "Urgent":
                #num_order.setStyleSheet("background-color: red; color: white;")
                #ident.setStyleSheet("background-color: red; color: white;")
                urgency.setStyleSheet("color: red; font-weight: bold")
                urgency.setText('High priority')
                cadrads.setText(order[i][2]+'*')
                cadrads.setStyleSheet('font-weight: bold')
            elif order[i][1] == "High priority":
                #num_order.setStyleSheet("background-color: orange; color: white;")
                #ident.setStyleSheet("background-color: orange; color: white;")
                urgency.setStyleSheet("color: orange; font-weight: bold")
                urgency.setText('Medium priority')
            else:
                #num_order.setStyleSheet("background-color: green; color: white;")
                #ident.setStyleSheet("background-color: green; color: white;")
            ...
            self.grid_layout.addWidget(info,i+2,0)
            self.grid_layout.addWidget(num_order,i+2,1)
            self.grid_layout.addWidget(ident,i+2,2)
            self.grid_layout.addWidget(urgency,i+2,4)
            self.grid_layout.addWidget(urg_std,i+2,3)
            self.grid_layout.addWidget(cadrads,i+2,5)
            self.grid_layout.addWidget(prepro,i+2,6)
            self.grid_layout.addWidget(cacs,i+2,7)
            self.grid_layout.addWidget(pretest,i+2,8)
            self.grid_layout.addWidget(motive,i+2,9)
    ...
    def on_button_clicked(self, label):
        ...
        report_coronaries.setGeometry(0,0,1050,900)
        report_coronaries.show()
    
    def update_window(self):
        self.close()
        
    def inactivate_validated_case(self,ident):
        print('ENTERED THE INACTIVATION FUNCTION')
        #ident = self.grid_layout.itemAtPosition(2,2).widget().text()
        for row in range(2,self.grid_layout.rowCount()):
            if self.grid_layout.itemAtPosition(row,2).widget().text() in ident: 
                print('ENTERED THE CHANGING FUNCTION')
                self.grid_layout.itemAtPosition(row,0).widget().setEnabled(False)
                self.grid_layout.itemAtPosition(row,1).widget().setStyleSheet("color: gray;")
                self.grid_layout.itemAtPosition(row,2).widget().setStyleSheet("color: gray;")
                self.grid_layout.itemAtPosition(row,4).widget().setStyleSheet("color: gray;")
                self.grid_layout.itemAtPosition(row,5).widget().setStyleSheet("color: gray;")
                self.grid_layout.itemAtPosition(row,6).widget().setStyleSheet("color: gray;")
                self.grid_layout.itemAtPosition(row,7).widget().setStyleSheet("color: gray;")
                self.grid_layout.itemAtPosition(row,8).widget().setStyleSheet("color: gray;")
                self.grid_layout.itemAtPosition(row,9).widget().setStyleSheet("color: gray;")
                self.grid_layout.itemAtPosition(row,3).widget().setStyleSheet("background-color: none; font-weight: normal")
                self.grid_layout.itemAtPosition(row,3).widget().setStyleSheet("color: gray;")
                self.grid_layout.itemAtPosition(row,10).widget().setEnabled(False)
"""
"""
ORDERED WORK LIST OF EACH PHYSICIAN (version 2)
"""
class WorkingList (QMainWindow):
#class LogIn(QWidget):
    def __init__(self, parent):
        super().__init__(parent)
        uic.loadUi('gui_worklist2.ui',self)
        ...
        for i in range(num_patients):
            ...
            self.grid_layout.addWidget(info,i+2,0)
            self.grid_layout.addWidget(num_order,i+2,1)
            self.grid_layout.addWidget(name,i+2,2)
            self.grid_layout.addWidget(urgency,i+2,4)
            self.grid_layout.addWidget(urg_std,i+2,3)
            self.grid_layout.addWidget(cadrads,i+2,5)
            self.grid_layout.addWidget(prepro,i+2,6)
            self.grid_layout.addWidget(motive,i+2,7)
            self.grid_layout.addWidget(action,i+2,8)
...
class ThreeDImageViewer(QMainWindow):
    def __init__(self, img_path, img_path_2):
        ...
        self.setWindowTitle("3D CCTA Image Viewer")
...
class ReportCoronaries(QMainWindow):
    def __init__(self,identifier,main_class):
        ...
        uic.loadUi('gui_report_coronaries.ui',self)
        ...
        #self.AI_info_label.setStyleSheet("background-color: purple; color: white;")
        ...
        if self.ident in self.main_class.get_imgAnalysis().keys():
            ...
            for i in range(len(self.database_segments)):
                for j in self.main_class.get_imgAnalysis()[self.ident]['Stenosis']:
                    #self.report_segments[i].setStyleSheet("background-color: purple; color: white;")
                    if self.main_class.get_imgAnalysis()[self.ident]['Stenosis'][j][1] == self.database_segments[i]:
                        ...
                        self.report_segments[i].setCurrentIndex(7)
    ...
    def changed_stenosis(self):
        ...
        if '100% Occluded' in modified_stenosis: 
            CADRADS = '5'
        elif modified_stenosis[0]=='100% Occluded' or modified_stenosis[0]=='70-99% Severe' or modified_stenosis[0]=='50-69% Moderate':
            CADRADS = '4B'
        elif modified_stenosis.count('70-99% Severe')>2:
            CADRADS = '4B'
        elif modified_stenosis.count('70-99% Severe')>0:
            CADRADS = '4A'
        elif modified_stenosis.count('50-69% Moderate')>0:
            CADRADS = '3'
        elif modified_stenosis.count('25-49% Mild')>0:
            CADRADS = '2'
        elif modified_stenosis.count('1-24% Minimal')>0:
            CADRADS = '1'
        else:
            CADRADS = '0'
        self.optionCADRADS.setCurrentIndex(self.CADRADS_options.index(CADRADS)+1)
    ...
    def changed_dominance(self):
        ...
        if prev == 1 and current == 2:
            self.optionRPD.setCurrentIndex(7)
            self.optionRPL.setCurrentIndex(7)
        ...
        #Case left to right
            ...
            self.optionLPD.setCurrentIndex(7)
            self.optionLPL.setCurrentIndex(7)
            self.previous_dominance = current
    ...
    def validate_report(self):
        ...
        final_report.setGeometry(0,0,1050,900)
        final_report.show()
...
class CPR_Viewer(QMainWindow):
    ...
    def changed_vessel(self):
        ...
        cpr_path = os.path.join(directory, 'Case1_ASOCA','CPR',vessel)
        cpr_image = cpr_path+'.png'
        #print('CPR path: ',cpr_image)
        pixmap = QPixmap(cpr_image)
        #pixmap = QPixmap(r"C:\Users\ferbe\Desktop\GUI\Case1_ASOCA\CPR\CPR_Cx.png")
...
class FinalReport(QMainWindow):
    def __init__(self,ident,main_class,textID,FemaleSex,MaleSex,textAge,UrgentStudy,textPhysician,textHospital,textService,dateStudy,optionDominance,optionCACs,optionLCA,optionpLAD,optionmLAD,optiondLAD,optionD1,optionD2,optionpCx,optiondCx,optionOM1,optionOM2,optionLPD,optionLPL,optionRI,optionpRCA,optionmRCA,optiondRCA,optionRPD,optionRPL,optionCADRADS,optionPlaque,optionN,optionHRP,optionIschemia,optionS,optionG,optionE,textEdit):
        super().__init__()
        uic.loadUi('gui_FINALreport_coronaries.ui',self)
        self.setWindowTitle('Final Report')
        self.ident = ident
        self.main_class = main_class
        
        self.textID.setText(textID.text())
        self.textID.setEnabled(False)
        self.textAge.setText(textAge.text())
        self.textAge.setEnabled(False)
        self.FemaleSex.setChecked(FemaleSex.isChecked())
        self.FemaleSex.setEnabled(False)
        self.MaleSex.setChecked(MaleSex.isChecked())
        self.MaleSex.setEnabled(False)
        self.UrgentStudy.setChecked(UrgentStudy.isChecked())
        self.UrgentStudy.setEnabled(False)
        self.textPhysician.setText(textPhysician.text())
        self.textPhysician.setEnabled(False)
        self.textHospital.setText(textHospital.text())
        self.textHospital.setEnabled(False)
        self.textService.setText(textService.text())
        self.textService.setEnabled(False)
        self.dateStudy.setDate(dateStudy.date())
        self.dateStudy.setDisabled(True)
        self.optionDominance.setCurrentIndex(optionDominance.currentIndex())
        self.optionDominance.setEnabled(False)
        self.optionCACs.setCurrentIndex(optionCACs.currentIndex())
        self.optionCACs.setEnabled(False)
        self.optionLCA.setCurrentIndex(optionLCA.currentIndex())
        self.optionLCA.setEnabled(False)
        self.optionpLAD.setCurrentIndex(optionpLAD.currentIndex())
        self.optionpLAD.setEnabled(False)
        self.optionmLAD.setCurrentIndex(optionmLAD.currentIndex())
        self.optionmLAD.setEnabled(False)
        self.optiondLAD.setCurrentIndex(optiondLAD.currentIndex())
        self.optiondLAD.setEnabled(False)
        self.optionD1.setCurrentIndex(optionD1.currentIndex())
        self.optionD1.setEnabled(False)
        self.optionD2.setCurrentIndex(optionD2.currentIndex())
        self.optionD2.setEnabled(False)
        self.optionpCx.setCurrentIndex(optionpCx.currentIndex())
        self.optionpCx.setEnabled(False)
        #self.optionmCx.setCurrentIndex(optionmCx.currentIndex())
        #self.optionmCx.setEnabled(False)
        self.optiondCx.setCurrentIndex(optiondCx.currentIndex())
        self.optiondCx.setEnabled(False)
        self.optionOM1.setCurrentIndex(optionOM1.currentIndex())
        self.optionOM1.setEnabled(False)
        self.optionOM2.setCurrentIndex(optionOM2.currentIndex())
        self.optionOM2.setEnabled(False)
        self.optionLPD.setCurrentIndex(optionLPD.currentIndex())
        self.optionLPD.setEnabled(False)
        self.optionLPL.setCurrentIndex(optionLPL.currentIndex())
        self.optionLPL.setEnabled(False)
        self.optionRI.setCurrentIndex(optionRI.currentIndex())
        self.optionRI.setEnabled(False)
        self.optionpRCA.setCurrentIndex(optionpRCA.currentIndex())
        self.optionpRCA.setEnabled(False)
        self.optionmRCA.setCurrentIndex(optionmRCA.currentIndex())
        self.optionmRCA.setEnabled(False)
        self.optiondRCA.setCurrentIndex(optiondRCA.currentIndex())
        self.optiondRCA.setEnabled(False)
        self.optionRPD.setCurrentIndex(optionRPD.currentIndex())
        self.optionRPD.setEnabled(False)
        self.optionRPL.setCurrentIndex(optionRPL.currentIndex())
        self.optionRPL.setEnabled(False)
        self.optionCADRADS.setCurrentIndex(optionCADRADS.currentIndex())
        self.optionCADRADS.setEnabled(False)
        self.optionPlaque.setCurrentIndex(optionPlaque.currentIndex())
        self.optionPlaque.setEnabled(False)
        self.optionN.setChecked(optionN.isChecked())
        self.optionN.setEnabled(False)
        self.optionHRP.setChecked(optionHRP.isChecked())
        self.optionHRP.setEnabled(False)
        self.optionIschemia.setCurrentIndex(optionIschemia.currentIndex())
        self.optionIschemia.setEnabled(False)
        self.optionS.setChecked(optionS.isChecked())
        self.optionS.setEnabled(False)
        self.optionG.setChecked(optionG.isChecked())
        self.optionG.setEnabled(False)
        self.optionE.setChecked(optionE.isChecked())
        self.optionE.setEnabled(False)
        self.textEdit.setText(textEdit.toPlainText())
        self.textEdit.setEnabled(False)
        
        #self.reported_vessels = [self.optionLCA,self.optionpLAD,self.optionmLAD,self.optiondLAD,self.optionpCx,self.optionmCx,self.optiondCx,self.optionD1,self.optionD2,self.optionOM1,self.optionOM2,self.optionLPD,self.optionLPL,self.optionRI,self.optionpRCA,self.optionmRCA,self.optiondRCA,self.optionRPD,self.optionRPL]
        ...
        #self.name_vessels = ['LCA','p-LAD','m-LAD','d-LAD','p-Cx','m-Cx','d-Cx','D1','D2','OM1','OM2','LPD','LPL','RI','p-RCA','m-RCA','d-RCA','RPD','RPL']
        self.name_vessels = ['LCA','p-LAD','m-LAD','d-LAD','p-Cx','d-Cx','D1','D2','OM1','OM2','LPD','LPL','RI','p-RCA','m-RCA','d-RCA','RPD','RPL']
        
        self.reported_degree = ['0% No CAD','1-24% Minimal','25-49% Mild','50-69% Moderate','70-99% Severe','100% Occluded','NA']
        ...
        self.pushButton_2.clicked.connect(self.graphical_results)
    ...
    def graphical_results(self):
        global results
        results = GraphicalResults(self.ident,self.main_class,self.optionLCA,self.optionpLAD,self.optionmLAD,self.optiondLAD,self.optionD1,self.optionD2,self.optionpCx,self.optiondCx,self.optionOM1,self.optionOM2,self.optionLPD,self.optionLPL,self.optionRI,self.optionpRCA,self.optionmRCA,self.optiondRCA,self.optionRPD,self.optionRPL)
        results.show()
...
"""                
class GraphicalResults(QMainWindow):
    def __init__(self,ident,main_class,optionLCA,optionpLAD,optionmLAD,optiondLAD,optionD1,optionD2,optionpCx,optiondCx,optionOM1,optionOM2,optionLPD,optionLPL,optionRI,optionpRCA,optionmRCA,optiondRCA,optionRPD,optionRPL):
        super().__init__()
        uic.loadUi('gui_graphical_results.ui',self)
        self.setWindowTitle('Graphical representation')
        directory = os.getcwd()
        coronaries = os.path.join(directory,'coronary_segments')
        coronaries = coronaries+'.png'
        #print('CPR path: ',cpr_image)
        pixmap = QPixmap(coronaries)
        #pixmap = QPixmap(r"C:\Users\ferbe\Desktop\GUI\Case1_ASOCA\CPR\CPR_Cx.png")
        self.label.setPixmap(pixmap)
        
        self.reported_degree = ['0% No CAD','1-24% Minimal','25-49% Mild','50-69% Moderate','70-99% Severe','100% Occluded','NA']
        self.vessels2report = [self.v1,self.v2,self.v3,self.v4,self.v5,self.v6,self.v7,self.v8,self.v9,self.v10,self.v11,self.v12,self.v13,self.v14,self.v15,self.v16,self.v17,self.v18]
        self.reported_vessels = [optionpRCA,optionmRCA,optiondRCA,optionRPD,optionLCA,optionpLAD,optionmLAD,optiondLAD,optionD1,optionD2,optionpCx,optionOM1,optiondCx,optionOM2,optionLPD,optionRPL,optionRI,optionLPL]
        
        for v in range(len(self.vessels2report)):
            self.vessels2report[v].setCurrentIndex(self.reported_vessels[v].currentIndex())
            if self.vessels2report[v].currentIndex() == 1:
                self.vessels2report[v].setStyleSheet("background-color: green; color: black;")
            elif self.vessels2report[v].currentIndex() == 2:
                self.vessels2report[v].setStyleSheet("background-color: yellow; color: black;")
            elif self.vessels2report[v].currentIndex() == 3:
                self.vessels2report[v].setStyleSheet("background-color: orange; color: black;")
            elif self.vessels2report[v].currentIndex() == 4:
                self.vessels2report[v].setStyleSheet("background-color: pink; color: black;")
            elif self.vessels2report[v].currentIndex() == 5:
                self.vessels2report[v].setStyleSheet("background-color: red; color: white;")
            elif self.vessels2report[v].currentIndex() == 6:
                self.vessels2report[v].setStyleSheet("background-color: black; color: white;")
            elif self.vessels2report[v].currentIndex() == 7:
                self.vessels2report[v].setStyleSheet("background-color: grey; color: black;")
            self.vessels2report[v].setEnabled(False)
